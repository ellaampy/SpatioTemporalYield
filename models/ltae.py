...
class LTAE(nn.Module):
    def __init__(self, in_channels=128, n_head=16, d_k=8, n_neurons=[512,128], dropout=0.2, d_model=512,
...
class LTAERegressor(nn.Module):
    """
    Lightweight Temporal Attention Encoder sequence regression
    """

    def __init__(self, input_dim=10,
                 n_head=16, d_k=8, d_model=256, n_neurons=[256, 128], dropout=0.2, T=1000, len_max_seq=24, positions=None,
                 decoder=[128, 64, 32, 1], return_att=False):
        super(LTAERegressor, self).__init__()
        self.temporal_encoder = LTAE(in_channels=input_dim, n_head=n_head, d_k=d_k,
                                           d_model=d_model, n_neurons=n_neurons, dropout=dropout,
                                           T=T, len_max_seq=len_max_seq, positions=positions, return_att=return_att
                                           )
        self.decoder = get_decoder(decoder)
        self.return_att = return_att
        self.modelname = 'LTAE_dk{}_Heads{}_neurons{}_T{}_dropout{}_decoder{}_positions{}'.format(d_k, n_head, '|'.join(list(map(str, n_neurons))), T,
                                                  dropout, '|'.join(list(map(str, decoder))), positions )


    def forward(self, input):
        """
         Args:
            input(tuple): TXC (time sequences of spectral channels
    ...
    def param_ratio(self):
        ...
        print('RATIOS: Temporal {:5.1f}% , Classifier {:5.1f}%'.format(t / total * 100, c / total * 100))
