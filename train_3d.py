...
from data_preparation.dataset_sampling import YDataset_Sampling
import torch
...
from models.models_3d import C3D
...
def main(cfg: DictConfig):
    ...
    device = torch.device(cfg.training.device)
     ...
     # Initialize Dataset with various parameters
    train_dataset = YDataset_Sampling(cfg.model_config.npy_path, cfg.dataset.label_path, 
                             ...
                             n_pixels=1024, transform=None, image_output=True)

    # Initialize Dataset with various parameters
    val_dataset = YDataset_Sampling(cfg.model_config.npy_path, cfg.dataset.label_path, 
                             norm_path= cfg.dataset.norm_path, lookup=cfg.dataset.train_years, 
    ...
    # Initialize Dataset with various parameters
    test_dataset_d = YDataset_Sampling(cfg.model_config.npy_path, cfg.dataset.label_path, 
    ...
    test_dataset_nd = YDataset_Sampling(cfg.model_config.npy_path, cfg.dataset.label_path, 
    ...
    if cfg.model_config.name == "c3d":
        model =  C3D(input_dim =cfg.dataset.input_dim, dropout=cfg.model_config.dropout)
        

    elif cfg.model_config.name == "convlstm":
        model = ConvLSTM(input_dim=cfg.dataset.input_dim, hidden_dim=cfg.model_config.hidden_dim, 
                        kernel_size=(cfg.model_config.kernel_size, cfg.model_config.kernel_size), 
                        num_layers=cfg.model_config.num_layers, batch_first=True, 
                        bias=cfg.model_config.bias, return_all_layers=False)
